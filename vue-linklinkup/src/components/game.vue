<template>
  <div :class="currentTheme.name">
    <div class="game-config">
      <button type="button" @click="init">重启</button>
    </div>
...
export default {
  ...
  methods: {
    ...
    initData () {
      ...
      // 先根据配置中的方块个数从方块数组中随机取出几条  配置除一行的数组
    ...
    handleClick (ev) {
      ...
      // 获取点击方块，如果方块是空的那么退出
      // cellIndex 属性可返回一行的单元格集合中单元格的位置。 获取行数
      // rowIndex 属性返回某一行在表格的行集合中的位置（row index）。 获取列数
      ...
      // Vue 实例重新渲染。注意它仅仅影响实例本身和插入插槽内容的子组件
      this.$forceUpdate()
    },
    // 选择方块
    selectCell (currCell) {
